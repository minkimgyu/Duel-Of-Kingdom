    ...
    class PacketHandler
    {
        ...
        public void HandlePacket(byte[] packet)
        {
            ...
            if(ClientTCP.Instance().buffer.Count() > 4)
            {
                ...
                if (dataLength > 4096)
                {
                    byte[] firstPacket = ClientTCP.Instance().buffer.ReadBytes(4096 - 4, true);
                    byte[] secondPacket = ClientTCP.Instance().packetToHandle.Dequeue();

                    // Concatenate segmentatedData and remainingData
                    Array.Copy(firstPacket, 0, data, 0, firstPacket.Length);
                    Array.Copy(secondPacket, 0, data, firstPacket.Length, secondPacket.Length);
                }
                else
                {
